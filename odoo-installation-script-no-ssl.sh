...
pre_run_checks() {
    ...
    if [ -z "$SERVER_IP_OR_HOSTNAME" ]; then read -p "Enter this server's Public IP Address (or a hostname): " SERVER_IP_OR_HOSTNAME; if [ -z "$SERVER_IP_OR_HOSTNAME" ]; then error "IP Address or hostname is required."; fi; fi
...
if [ -n "$CUSTOM_ADDONS_REPO" ]; then
    git clone --depth 1 "$CUSTOM_ADDONS_REPO" custom_addons
else
    mkdir custom_addons
fi
python3 -m venv venv
...
deactivate
EOF
# <-- CORRECTED: The closing EOF is now correctly on its own line.
...
[Unit]
Description=Odoo 18 Enterprise (No-SSL)
...
server {
    listen 80;
    server_name ${SERVER_IP_OR_HOSTNAME} _;
